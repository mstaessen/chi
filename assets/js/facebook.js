...
function requestFacebookMessages(){
	if(storage.getItem("facebookToken")!=null){
		...
		var queryParams = [accessToken,"since="+storage.getItem('facebookUpdateTime'),until,'callback=loadFacebookMessages'];
		var query = queryParams.join('&');
		var url = path + query;
...
var newMsg={
	...
	location: 'inbox',
    read_later: -1,
    labels: [{ label: 'facebook', pretty: 'Facebook' }]
};
newMsg.original=msg;
...
if(msg.message!=null){
	...
	newMsg.short_content=msg.message.slice(0,50)+'...';
	addMessage(newMsg);
}
